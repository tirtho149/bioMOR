"""Train Pathformer on a single fold of the 3-modality BRCA PAM50 dataset.

Wraps the *unmodified* `pathformer_model`. With N_PRJ == 3 we use the native
no-embedding code path (embeding=False, row_dim=N_PRJ), the same branch the
original uses for N_PRJ > 2.

Differences from Pathformer_train_2mod.py:
  - Always emits per-CLASS softmax probabilities (N, 5) needed for multiclass
    AUC / AUPR.  The saved test_predictions CSV stores the full probability
    vector per sample (JSON-encoded), and the per-fold result JSON stores the
    softmax probabilities so the orchestrator can build the shared metrics JSON.

All training hyperparameters are unchanged from the original.
"""
...
def evaluate(model, loader, pathway_network, device, n_classes):
    """Returns (metrics, y_true, probs) where probs is (N, n_classes) softmax."""
    model.eval()
    ys, logits_all = [], []
    with torch.no_grad():
        for x, y in loader:
...
def train_one_fold(args):
    ...
    out = {
        ...
        "n_classes": n_classes,
        # Per-class softmax probabilities + labels so the orchestrator can build
        # the shared multiclass metrics JSON.
        "y_true": y_true.astype(int).tolist(),
        "y_prob": p_test.astype(float).tolist(),
        "args": vars(args),
        "history": history,
    }
    ...
    # Always store full per-class probability vector per sample.
    preds_df = pd.DataFrame({
        "fold": args.fold,
        "sample": test_ids,
        "y_label": y_true,
        "y_prob": [json.dumps(list(map(float, row))) for row in p_test],
    })
...
def main():
    ...
    ap.add_argument("--data_dir", required=True, help="Output dir of pathformer_preprocess_3mod.py")
