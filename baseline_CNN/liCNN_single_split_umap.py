...
class LateIntegrationCNN(torch.nn.Module):
    """Late Integration CNN baseline architecture."""
    ...
    def forward(self, x, in_dim, num_classes):
        x_mut = x[:, : in_dim[0]].unsqueeze(1)
        x_cnv = x[:, in_dim[0] : in_dim[0] + in_dim[1]].unsqueeze(1)
...
class MultiOmicsDataset(Dataset):
    """Dataset wrapper for integrated omics data."""
...
class EarlyStopping:
    ...
    def __init__(self, patience=25, delta=0.0, stop=0):
...
def compute_metrics_with_threshold(y_true, y_scores):
    """Compute binary metrics using fixed threshold 0.5."""
    y_true = np.array(y_true)
    y_scores = np.array(y_scores)
    y_pred = (y_scores >= 0.5).astype(int)

    auc_val = roc_auc_score(y_true, y_scores) if len(np.unique(y_true)) > 1 else np.nan
    aupr_val = average_precision_score(y_true, y_scores) if len(np.unique(y_true)) > 1 else np.nan
...
def calculate_comprehensive_metrics(y_true, y_pred_proba):
    """Return full metric dict for one split."""
    m = compute_metrics_with_threshold(y_true, y_pred_proba)
    return {
        "AUC_ROC": m["auc"],
        "AUC_PR": m["aupr"],
        "Accuracy": m["acc"],
...
def load_and_prepare_data():
    """Load and prepare multi-omics data for liCNN."""
    ...
    assert mutation_matrix.shape[0] == cnv_matrix.shape[0] == len(labels), "Dataset sample mismatch"
...
def _safe_2d_projection(embeddings, method="umap", random_state=42):
    """Project embeddings to 2D; UMAP with TSNE/PCA fallback."""
    ...
    method = method.lower()
    if method != "umap":
        raise ValueError(f"Unsupported projection method: {method}. This script only supports UMAP.")

    try:
        import umap  # type: ignore
    except ImportError as exc:
        raise ImportError("UMAP projection requires the `umap-learn` package to be installed.") from exc
    ...
    try:
        ...
        raise RuntimeError("UMAP projection failed.") from exc


def analyze_embedding_separation(embeddings, y_true, sample_ids, outdir, split_name, random_state=42):
    """Save projection and class-separation plots/statistics."""
    ...
    class_names = np.array(["Primary" if int(v) == 0 else "Metastatic" for v in y_true])
    class_palette = {
        "Primary": "#81f411",
        "Metastatic": "#fd2300",
    }
...
def _collect_probs_and_labels(model, loader, device, feature_dims, num_classes=2):
    """Collect positive-class probabilities and labels from loader."""
    model.eval()
    probs = []
    labels = []
...
def _collect_embeddings(model, loader, device, feature_dims):
    """Collect penultimate liCNN embeddings (10-d before final classifier)."""
    model.eval()
    pin_memory = device.type == "cuda"
    embeddings = []
    ...
    with torch.no_grad():
        for data, _ in loader:
            ...
            x_mut = data[:, : feature_dims[0]].unsqueeze(1)
            x_cnv = data[:, feature_dims[0] : feature_dims[0] + feature_dims[1]].unsqueeze(1)

            x_1 = model.FC_1(x_mut)
            x_2 = model.FC_2(x_cnv)
            x = torch.cat((x_1, x_2), 1)
            emb = model.FC_merge[:-1](x)
...
def train_late_integration_cnn_single_split(
    integrated_data,
    labels,
    feature_dims,
    patient_ids,
    output_dir="late_integration_cnn_single_split_results",
    random_state=42,
):
    """Train liCNN once using a 70/10/20 stratified split."""
    print("Training Late Integration CNN with single split (70/10/20)...")
    ...
    train_eval_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=False, pin_memory=pin_memory)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, pin_memory=pin_memory)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, pin_memory=pin_memory)

    model = LateIntegrationCNN(in_dim=feature_dims, num_classes=num_classes).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        y_val_prob, y_val_true = _collect_probs_and_labels(model, val_loader, device, feature_dims, num_classes)
        y_val_eval = compute_metrics_with_threshold(y_val_true, y_val_prob)
        val_f1_macro = y_val_eval["f1_macro"]
    ...
    y_train_prob, y_train_true = _collect_probs_and_labels(model, train_eval_loader, device, feature_dims, num_classes)
    y_val_prob, y_val_true = _collect_probs_and_labels(model, val_loader, device, feature_dims, num_classes)
    y_test_prob, y_test_true = _collect_probs_and_labels(model, test_loader, device, feature_dims, num_classes)

    train_metrics = calculate_comprehensive_metrics(y_train_true, y_train_prob)
    valid_metrics = calculate_comprehensive_metrics(y_val_true, y_val_prob)
    test_metrics = calculate_comprehensive_metrics(y_test_true, y_test_prob)
    ...
    print(
        ...
        f"Precision={valid_metrics['Precision']:.4f}, Recall={valid_metrics['Recall']:.4f}"
    )
    print(
        "Test  | "
    ...
    train_embeddings = _collect_embeddings(model, train_eval_loader, device, feature_dims)
    test_embeddings = _collect_embeddings(model, test_loader, device, feature_dims)
    ...
    preds_csv = os.path.join(output_dir, "late_integration_cnn_test_predictions.csv")
    preds_df.to_csv(preds_csv, index=False)

    metrics_df = pd.DataFrame({"Train": train_metrics, "Valid": valid_metrics, "Test": test_metrics}).T
    metrics_csv = os.path.join(output_dir, "late_integration_cnn_single_split_metrics.csv")
    ...
    final_results = {
        ...
        "configuration": {
            ...
            "feature_dimensions": [int(feature_dims[0]), int(feature_dims[1])],
        },
        "dataset": {
    ...
    results_json = os.path.join(output_dir, "late_integration_cnn_single_split_results.json")
...
def main():
    """Main pipeline for liCNN single split + UMAP separation."""
    print("=" * 60)
    print("Late Integration CNN - Single Split (70/10/20)")
    print("=" * 60)

    output_dir = "late_integration_cnn_single_split_results"
    os.makedirs(output_dir, exist_ok=True)

    integrated_data, labels, feature_dims, _, patient_ids = load_and_prepare_data()

    train_late_integration_cnn_single_split(
