...
PLOT_DPI = 600
PLOT_FONT_SIZE = 12
CLASS_LABEL_MAP = {0: "Primary", 1: "Metastatic"}
CLASS_COLOR_MAP = {"Primary": "#81f411", "Metastatic": "#fd2300"}
...
def analyze_embedding_separation(embeddings, y_true, sample_ids, outdir, split_name, random_state=42):
    ...
    class_names = np.array([CLASS_LABEL_MAP.get(int(v), f"Class {int(v)}") for v in y_true])
...
def train_pathcnn_single_split(pathway_images, labels, random_state=42):
    ...
    def evaluate_split(split_name, X_split, y_split):
        y_prob = model.predict(X_split, verbose=0)[:, 1]
    ...
    print("\nFinal metrics:")
    train_metrics = evaluate_split("Train", X_train, y_train)
    val_metrics = evaluate_split("Valid", X_val, y_val)
    test_metrics = evaluate_split("Test", X_test, y_test)

    feature_model = Model(inputs=model.inputs, outputs=model.get_layer("embedding_dense").output)
    train_embeddings = feature_model.predict(X_train, verbose=0)
    test_embeddings = feature_model.predict(X_test, verbose=0)
