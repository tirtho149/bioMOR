...
def load_and_preprocess_data():
    ...
    return mutation_matrix, cnv_matrix, labels, gene_names, pathway_genes, patient_ids
...
def train_pathcnn_5fold_cv(pathway_images, labels, patient_ids, n_splits=5, random_state=42):
    """
    Train PathCNN using 5-fold stratified cross-validation
    """
    print(f"Training PathCNN with {n_splits}-fold cross-validation (random_state={random_state})...")
    ...
    for fold_idx, (train_val_index, test_index) in enumerate(skf.split(X, y)):
        print(f"\n{'='*60}")
        ...
        patient_train_val, patient_test = patients[train_val_index], patients[test_index]
        
        # Further split train_val into train and validation (90/10 split)
        ...
        fold_pred_path = os.path.join('results', f'fold_{fold_idx+1}_test_predictions.csv')
        fold_pred_df.to_csv(fold_pred_path, index=False)
        all_fold_predictions.append(fold_pred_df.reset_index(drop=True))
        
        # Calculate all metrics
        fold_metrics = calculate_metrics(y_test, y_pred_proba, y_pred_binary)
        ...
        with open(f'results/fold_{fold_idx+1}_results.json', 'w') as f:
    ...
    for metric_name in ['auc', 'aupr', 'accuracy', 'f1', 'precision', 'recall']:
        if all_metrics[metric_name]:
            mean_std[metric_name] = {
                ...
                "std": float(np.std(all_metrics[metric_name], ddof=1))  # Sample std
            }
    
    print("\n" + "="*60)
    print(f"FINAL RESULTS ({n_splits}-Fold Cross-Validation)")
    ...
    pd.DataFrame(mean_std).T.to_csv("results/cv_summary_mean_std.csv", index_label="metric")
    
    with open("results/cv_summary_mean_std.json", "w") as f:
    ...
    for metric_name, values in all_metrics.items():
        if values:
            ...
            results_summary[f'{metric_name}_std'] = np.std(values, ddof=1)
    ...
    with open('results/pathcnn_5fold_cv_results.json', 'w') as f:
        json.dump(final_results, f, indent=4)
    
    print(f"\nAll results saved in 'results' directory:")
    print(f"  - results/cv_summary_mean_std.csv")
    print(f"  - results/cv_summary_mean_std.json")
    print(f"  - results/pathcnn_5fold_cv_results.json (comprehensive)")
    print(f"  - results/fold_[1-{n_splits}]_results.json (individual folds)")

    # Save combined predictions from all folds in one file
    if all_fold_predictions:
        combined_preds = pd.concat(all_fold_predictions, ignore_index=True)
        combined_path = os.path.join('results', 'all_fold_test_predictions.csv')
        combined_preds.to_csv(combined_path, index=False)
        print(f"  - {combined_path} (all fold predictions)")
    
    return final_results

def main():
    """
    Main function to run PathCNN pipeline with 5-fold CV
    """
    print("Starting PathCNN pipeline with 5-fold cross-validation...")
    
    # Load and preprocess data
    mutation_data, cnv_data, labels, gene_names, pathway_genes, patient_ids = load_and_preprocess_data()
    
    # Create pathway-gene mapping
    pathway_mapping, pathway_names = create_pathway_gene_mapping(pathway_genes, gene_names)
    
    # Apply PCA to pathways for each omics type
    ...
    # Train PathCNN with 5-fold cross-validation
    final_results = train_pathcnn_5fold_cv(ordered_pathway_images, labels, patient_ids,
