...
# FULLY GPU-ACCELERATED VERSION with Diagnostics
...
print('='*80)
print('GPU-OPTIMIZED MOGAT with 5-Fold CV - FULLY ACCELERATED VERSION')
print('='*80)
...
def setup_gpu():
    """Setup GPU with proper configuration and diagnostics"""
    ...
    if torch.cuda.is_available():
        ...
        torch.backends.cudnn.enabled = True  # CRITICAL: Enable cuDNN
        torch.backends.cudnn.benchmark = True  # Auto-tune for best performance
        torch.backends.cudnn.deterministic = False  # For reproducibility, set to True
        ...
        print(f"  - Benchmark mode: ON")
        print(f"  - Memory: {torch.cuda.memory_allocated(0) / 1e9:.2f} GB allocated")
        
    else:
        print(f"✗ CUDA NOT AVAILABLE")
        print(f"⚠ Running on CPU - Training will be MUCH slower!")
        print(f"\nTo fix:")
        print(f"  1. Check GPU: nvidia-smi")
        print(f"  2. Check PyTorch CUDA: python -c 'import torch; print(torch.cuda.is_available())'")
        print(f"  3. Reinstall PyTorch with CUDA support")
    
    print(f"{'='*80}\n")
    return device

def verify_gpu_tensor(tensor, name="tensor"):
    """Verify tensor is on GPU and print diagnostic info"""
    if torch.cuda.is_available():
        if tensor.is_cuda:
            print(f"  ✓ {name}: ON GPU ({tensor.device}), shape: {tensor.shape}")
            return True
        else:
            print(f"  ✗ WARNING: {name} is on CPU! Moving to GPU...")
            return False
    return True
...
class MLPClassifierGPU(nn.Module):
    ...
    def __init__(self, input_size, hidden_sizes, num_classes=2, dropout=0.5):
...
# ================================================================================
# TRAINING FUNCTIONS (FULLY GPU-ACCELERATED)
# ================================================================================
...
def train_gat(model, data, optimizer, criterion, device):
    ...
    assert data.x.is_cuda, "Features not on GPU!"
    assert data.edge_index.is_cuda, "Edge index not on GPU!"
    assert data.y.is_cuda, "Labels not on GPU!"
...
def train_classifier_gpu(model, X_train, y_train, X_valid, y_valid, 
    ...
    if not y_train.is_cuda:
        y_train = y_train.to(device, non_blocking=True)
    if not X_valid.is_cuda:
        X_valid = X_valid.to(device, non_blocking=True)
    if not y_valid.is_cuda:
    ...
    for epoch in range(max_epochs):
        ...
        # Training with mini-batches (on GPU)
        indices = torch.randperm(X_train.shape[0], device=device)  # GPU shuffle
...
if not torch.cuda.is_available():
    ...
    print("Please run on a GPU-enabled machine or check your PyTorch installation.")
    # Don't exit, but warn heavily
    print("CONTINUING ON CPU - Performance will be significantly degraded!")
...
output_dir = base_path + f"MOGAT_{dataset_name}_5fold_GPU_results_{timestamp}/"
...
# Move labels to GPU IMMEDIATELY
labels_gpu = torch.tensor(labels_cpu, dtype=torch.long, device=device)
verify_gpu_tensor(labels_gpu, "labels_gpu")
...
for fold_idx, (train_valid_idx, test_idx) in enumerate(skf.split(np.arange(len(labels_cpu)), labels_cpu)):
    ...
    clear_gpu_memory()
    print_gpu_memory_usage("Start of fold")
    ...
    # Before building fold_data
    train_labels, train_counts = np.unique(labels_cpu[train_idx], return_counts=True)
    valid_labels, valid_counts = np.unique(labels_cpu[valid_idx], return_counts=True)
    test_labels,  test_counts  = np.unique(labels_cpu[test_idx],  return_counts=True)
    ...
    fold_data = {
        ...
        'split_info': {
            ...
            'train_label_dist': {int(k): int(v) for k, v in zip(train_labels, train_counts)},
            'valid_label_dist': {int(k): int(v) for k, v in zip(valid_labels, valid_counts)},
            'test_label_dist':  {int(k): int(v) for k, v in zip(test_labels,  test_counts)},
        },
        'embeddings_trained': {},
        'classification_results': []
    }
    ...
    # ============================================================================
    # LOAD AND PREPARE FEATURES (DIRECTLY TO GPU)
    # ============================================================================
    
    print(f'\nLoading features DIRECTLY to GPU...')
    is_first = 0
    ...
    print(f"\nCombined features shape: {new_x.shape}")
    verify_gpu_tensor(new_x, "new_x (combined features)")
    print_gpu_memory_usage("After loading features")
    
    # ============================================================================
    # TRAIN EMBEDDINGS FOR EACH NETWORK (FULLY ON GPU)
    # ============================================================================
    ...
    for n in range(len(node_networks)):
        ...
        with open(edge_file, 'rb') as f:
            edge_index_df = pickle.load(f)
        
        print(f"Edge index DataFrame shape: {edge_index_df.shape}")
        ...
        for learning_rate in learning_rates:
            for hid_size in hid_sizes:
                ...
                for ii in range(xtimes2):
                    ...
                    # Prepare edge data - DIRECTLY TO GPU
                    edge_index_vals = edge_index_df.iloc[:, 0:2].values
                    edge_index_tensor = torch.tensor(
                        edge_index_vals.T,  # Transpose to get [2, num_edges]
                        dtype=torch.long,
                        device=device
                    )
                    
                    # Edge attributes - DIRECTLY TO GPU
                    edge_attr_vals = edge_index_df.iloc[:, 2].values
                    edge_attr_tensor = torch.tensor(
                        edge_attr_vals,
                        dtype=torch.float32,
                        device=device
                    ).unsqueeze(1)  # Make it [num_edges, 1] for GATConv
                    
                    verify_gpu_tensor(edge_index_tensor, "edge_index")
                    verify_gpu_tensor(edge_attr_tensor, "edge_attr")
                    
                    # Create Data object - ALREADY ON GPU
                    data = Data(
                        x=new_x,
                    ...
                    # Create masks - DIRECTLY ON GPU
                    train_mask = torch.zeros(data.x.shape[0], dtype=torch.bool, device=device)
                    train_mask[train_idx] = True
                    
                    valid_mask = torch.zeros(data.x.shape[0], dtype=torch.bool, device=device)
                    valid_mask[valid_idx] = True
                    
                    data.train_mask = train_mask
                    data.valid_mask = valid_mask
                    
                    # Verify all data is on GPU
                    print(f"  Data verification:")
                    verify_gpu_tensor(data.x, "data.x")
                    verify_gpu_tensor(data.edge_index, "data.edge_index")
                    verify_gpu_tensor(data.edge_attr, "data.edge_attr")
                    verify_gpu_tensor(data.y, "data.y")
                    verify_gpu_tensor(data.train_mask, "data.train_mask")
                    verify_gpu_tensor(data.valid_mask, "data.valid_mask")
        ...
        # ========================================================================
        # TRAIN FINAL GAT MODEL (FULLY ON GPU)
        # ========================================================================
        
        print(f"\nTraining final GAT model...")
        print(f"  Best LR: {best_emb_lr}, Best Hidden: {best_emb_hs}")
        ...
        # Save embedding (move to CPU for saving)
        ...
        print_gpu_memory_usage("After GAT training")
        
        del model, optimizer, data, emb
        clear_gpu_memory()
    
    # ============================================================================
    # INTEGRATION AND CLASSIFICATION (FULLY ON GPU)
    # ============================================================================
    
    print(f"\n{'='*80}")
    print("INTEGRATION AND CLASSIFICATION (100% GPU-ACCELERATED)")
    print(f"{'='*80}")
    ...
    for trials in range(len(trial_combs)):
        ...
        verify_gpu_tensor(emb, "Combined embeddings")
        
        # Prepare datasets (ALL ON GPU)
        ...
        verify_gpu_tensor(X_train, "X_train")
        verify_gpu_tensor(y_train, "y_train")
        print_gpu_memory_usage("After dataset preparation")
        
        # ========================================================================
        # MODEL TRAINING (100% GPU-ACCELERATED)
        # ========================================================================
        ...
        if int_method == 'MLP':
            ...
            print(f"  MLP model on device: {next(model.parameters()).device}")
    ...
    print_gpu_memory_usage("End of fold")
    clear_gpu_memory()
...
for comb_num, comb_data in combinations_summary.items():
    ...
    for metric in ['accuracy', 'auc', 'aupr', 'precision_binary', 'recall_binary', 'f1_binary']:
        ...
        summary_stat['test_performance_across_folds'][metric] = {
            ...
            'std': round(np.std(values, ddof=1), 4),  # Sample std
...
json_file = output_dir + f"MOGAT_{dataset_name}_5fold_GPU_complete.json"
summary_csv = output_dir + f"MOGAT_{dataset_name}_5fold_GPU_summary.csv"
...
print(f"\n{'='*80}")
print(f"FINAL SUMMARY - 5-FOLD CV COMPLETED")
print(f"{'='*80}")
...
print(f"  Integration: {int_method}")

print(f"\nOverall Performance (Mean ± Std across {n_folds} folds):")
print(f"  Best AUC:       {master_results['summary']['overall_statistics']['best_test_auc']:.4f}")
print(f"  Best F1:        {master_results['summary']['overall_statistics']['best_test_f1']:.4f}")

print(f"\nBest Combinations by Metric:")
for metric, info in best_by_metric.items():
    print(f"  {metric}: {info['mean_score']:.4f} ± {info['std_score']:.4f} → {info['used_embeddings']}")
...
if torch.cuda.is_available():
    ...
    print(f"  ✓ 100% GPU ACCELERATED")

print(f"\n{'='*80}")
print("✓ GPU-OPTIMIZED MOGAT WITH 5-FOLD CV COMPLETED SUCCESSFULLY!")
print(f"All operations executed on GPU: ✓ GAT ✓ Classification ✓")
