...
from utils.evaluate import (evalualte_classification_binary,
                            evalualte_classification_multiclass,
                            evalualte_regression, evalualte_survival)
...
class OneSplitPipeline:
    ...
    def run(self):
        ...
        for data_params in self.data_params:
            ...
            if self.eval_dataset == 'validation':
                ...
                info_t = np.concatenate((np.asarray(info_test_), np.asarray(info_validate_)))
    ...
    def evaluate(self, y_test, y_pred_test, y_pred_test_scores):
        ...
        if self.task == 'classification':
            test_score = evalualte_classification_multiclass(y_test, y_pred_test, y_pred_test_scores)
    ...
    def predict(self, model, x_test, y_test):
        ...
        if self.task == 'classification':
            # Multiclass: keep the full (N, K) softmax matrix as scores.
