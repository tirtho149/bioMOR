...
def get_deep_explain_score_layer(model, X, layer_name, output_index=-1, method_name='grad*input'):
    ...
    The original implementation relied on TF1 graph-mode `tf.Session()` and
    the DeepExplain library (TF1-only). Both are removed in the slim port.
    To use feature-importance with this codebase, either:
      (a) implement an equivalent using `tf.GradientTape` and `tf.gradients`
          in eager mode, or
      (b) pin TF1 in a separate conda environment.
    Set `feature_importance: None` in your param file to skip this entirely
    and only get classification metrics + y_score (the slim port's default).
    """
    raise NotImplementedError(
        "get_deep_explain_score_layer is disabled in the TF2 port. "
        "Set feature_importance=None in your param file, or re-implement "
        "this function using tf.GradientTape."
    )
