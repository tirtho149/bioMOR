...
class Model(BaseEstimator):
    ...
    def get_callbacks(self, X_train, y_train):
        ...
        if self.select_best_model:
            ckpt_path = self.save_filename if self.save_filename.endswith('.keras') else self.save_filename + '.keras'
            saving_callback = ModelCheckpoint(ckpt_path, monitor=self.monitor, verbose=1, save_best_only=True,
                                              mode='max')
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        if self.class_weight == 'auto':
            ...
            class_weights = class_weight.compute_class_weight('balanced', classes=classes, y=y_train.ravel())
        ...
        # Keras 3 disallows `class_weight` on multi-output models. Convert to
        # per-sample weights so the same balancing applies across all heads.
        sample_weight = None
        if isinstance(class_weights, dict):
            y_flat = np.asarray(y_train).ravel()
            sample_weight = np.array([class_weights[int(c)] for c in y_flat],
                                     dtype=np.float32)
        ...
        if self.n_outputs > 1:
            ...
            if sample_weight is not None:
                sample_weight = [sample_weight] * self.n_outputs
        ...
        if self.max_f1 and self.loss != 'sparse_categorical_crossentropy':
            # Binary-only threshold search; skip for multi-class softmax output.
    ...
    def predict(self, X_test):
        if self.select_best_model:
            ckpt_path = self.save_filename if self.save_filename.endswith('.keras') else self.save_filename + '.keras'
            logging.info("loading model %s" % ckpt_path)
            self.model.load_weights(ckpt_path)

        prediction_scores = self.get_prediction_score(X_test)

        # Multi-class: argmax along the class axis.
        if self.loss == 'sparse_categorical_crossentropy' or (
                prediction_scores.ndim == 2 and prediction_scores.shape[1] > 1):
            return np.argmax(prediction_scores, axis=1).astype(int)
    ...
    def predict_proba(self, X_test):
        prediction_scores = self.get_prediction_score(X_test)
        # Multi-class: model already emits a softmax (N, K) matrix.
        if self.loss == 'sparse_categorical_crossentropy' or (
                prediction_scores.ndim == 2 and prediction_scores.shape[1] > 1):
            return np.asarray(prediction_scores)
    ...
    def save_model(self, filename):
        ...
        weights_path = filename if filename.endswith('.weights.h5') else filename.replace('.h5', '.weights.h5')
