#!/bin/bash
# Smoke-run every survival baseline (SMOKE=1: 1 fold, few epochs), one by one,
# on one GPU. Torch baselines use the `path` env; PathCNN (Keras) uses `pnet`.
...
run() {
  ...
  t0=$(date +%s); SMOKE=1 PYTHONPATH="$DIR" "$@"; rc=$?; t1=$(date +%s)
...
echo ""; echo "############## ALL SURVIVAL SMOKE RUNS DONE ##############"
