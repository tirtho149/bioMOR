...
% ==================================================================
% 3. PANEL B NODES  (blocks pushed ~0.4 row down, clear of header band)
% ==================================================================
...
% --- n_mor ---
\node[stageboxB,minimum width=2.00cm,minimum height=1.10cm] (n_mor) at (13.20,4.15) {};
\node[accentB] at (13.20,4.52) {\bfseries\footnotesize MoR depth router};
\foreach \i/\h in {0/0.16,1/0.34,2/0.24,3/0.42}{
  \fill[accentB] (12.72+\i*0.30,3.85) rectangle (12.90+\i*0.30,3.85+\h);
}
\node[accentB] at (13.85,3.99) {\fontsize{6.5}{7.5}\selectfont\bfseries $d_m$};
\node[accentB] at (13.20,3.75) {\fontsize{6.5}{7.5}\selectfont\bfseries primed by graph};
...
% Panel B
% a6 BOLD weight-shared loop over the shared block (clearance above f_theta)
\draw[draw=accentB,line width=1.9pt,-{Stealth[length=6pt,width=5pt]}]
  (9.95,4.70) .. controls (10.00,5.35) and (11.20,5.35) .. (11.25,4.70);
\node[accentB] at (10.60,5.42) {\itshape\bfseries\fontsize{8.5}{9.5}\selectfont weight-shared $\times K$};
% a7 block <-> mor
\draw[<->,line width=0.9pt,draw=accentB] (n_block.east) -- (n_mor.west);
% a8 priming arrow (top margin, from graph to MoR; clears header band)
\draw[draw=accentA,line width=0.5pt,opacity=0.40,->,dash pattern=on 3pt off 2pt]
  (4.35,6.15) .. controls (8.00,6.72) and (11.50,6.72) .. (13.75,4.78);
\node[accentB,opacity=0.75] at (14.30,5.15) {\itshape\fontsize{7}{8}\selectfont $+\,\beta_t\!\cdot\!\pi_m$};
% a9 block -> pool
...
% ==================================================================
% 5. RESULTS BAR CHART  (redesigned)
% scale: 50->0.56, 60->1.00, 70->1.44 ; 0.044 cm per %
% ==================================================================
% --- chart title on its OWN line above the plot ---
\node[ink,anchor=west] at (9.95,1.97)
  {\bfseries\fontsize{7}{8}\selectfont Gene-graph ablation --- single-cell macro-F1};

% --- y axis ---
\draw[muted,line width=0.4pt] (9.95,0.56) -- (9.95,1.44);
\foreach \p/\yy in {50/0.56,60/1.00,70/1.44}{
  \draw[muted,line width=0.4pt] (9.90,\yy) -- (9.95,\yy);
  \node[muted,anchor=east] at (9.87,\yy) {\fontsize{6.5}{7.5}\selectfont \p};
}

% --- bars (width 0.44; centers 10.34 / 11.26 / 12.18 / 13.10) ---
% none 57.5 -> 0.89
\fill[muted] (10.12,0.56) rectangle (10.56,0.89);
% random 57.5 -> 0.89
\fill[muted] (11.04,0.56) rectangle (11.48,0.89);
% fixed ~57 -> 0.868 (hollow)
\draw[draw=muted,line width=0.6pt,fill=white] (11.96,0.56) rectangle (12.40,0.868);
% LEARNED 70.8 -> 1.475
\fill[accentA] (12.88,0.56) rectangle (13.32,1.475);
...
\draw[muted,line width=0.5pt,dash pattern=on 2pt off 2pt,opacity=0.75]
  (10.12,0.89) -- (12.40,0.89);

% --- per-bar value labels (directly above each own bar) ---
\node[muted] at (10.34,1.00) {\fontsize{7}{8}\selectfont\bfseries 57.5};
\node[muted] at (11.26,1.00) {\fontsize{7}{8}\selectfont\bfseries 57.5};
\node[muted] at (12.18,0.98) {\fontsize{7}{8}\selectfont\bfseries $\approx$57};
\node[accentA] at (13.10,1.60) {\fontsize{7.5}{8.5}\selectfont\bfseries 70.8};

% --- honest-negative tag over the flat bars, arrow to the fixed bar ---
\node[ink,align=center] at (11.20,1.52)
  {\fontsize{7}{8}\selectfont\bfseries honest negative\\[0.6pt]\fontsize{6.3}{7}\selectfont (no gain over random)};
\draw[muted,line width=0.5pt,->] (11.80,1.33) .. controls (12.02,1.12) .. (11.98,0.92);

% --- x-axis category labels (spaced, enlarged; only LEARNED green) ---
\node[muted] at (10.34,0.42) {\fontsize{7.5}{8.5}\selectfont none};
\node[muted] at (11.26,0.42) {\fontsize{7.5}{8.5}\selectfont random};
\node[muted] at (12.18,0.42) {\fontsize{7.5}{8.5}\selectfont fixed};
\node[accentA] at (13.10,0.42) {\fontsize{7.5}{8.5}\selectfont\bfseries LEARNED};

% --- +11.7 vertical bracket on the RIGHT (57.5 -> 70.8), label beside ---
\draw[accentA,line width=0.9pt] (13.52,0.89) -- (13.52,1.475);
\draw[accentA,line width=0.9pt] (13.45,0.89) -- (13.59,0.89);
\draw[accentA,line width=0.9pt] (13.45,1.475) -- (13.59,1.475);
\node[accentA,anchor=west] at (13.64,1.18) {\bfseries\fontsize{8.5}{9.5}\selectfont +11.7};

% --- efficiency badges : fully RIGHT of the plotting area, stacked ---
\draw[draw=accentB,line width=0.7pt,rounded corners=4pt,fill=white]
  (14.55,1.14) rectangle (15.66,1.76);
\node[accentB,align=center] at (15.105,1.56) {\bfseries\fontsize{7}{8}\selectfont 4$\times$ fewer};
\node[muted,align=center] at (15.105,1.32) {\fontsize{6}{7}\selectfont params (K=4)};
\draw[draw=accentB,line width=0.7pt,rounded corners=4pt,fill=white]
  (14.55,0.46) rectangle (15.66,1.08);
\node[accentB,align=center] at (15.105,0.88) {\bfseries\fontsize{7}{8}\selectfont $\sim$38\% fewer};
\node[muted,align=center] at (15.105,0.64) {\fontsize{6}{7}\selectfont FLOPs, matched};
