...
for each of the three models (Independent / Recursive-shared / MoR) across all six TCGA tasks;
recurrent-block parameters are listed under each model. The shared recursive model matches the
$\sim$4$\times$-larger Independent stack within a couple of accuracy points on most tasks ---
and even edges it on \texttt{pathologic\_stage} and \texttt{tumor\_status} --- at one quarter
...
\texttt{pathologic\_stage} & 4 & 548 & 0.4909 & \best{0.5000} & 0.4745 \\
\texttt{pathologic\_T}     & 4 & 548 & \best{0.4653} & 0.4580 & 0.4288 \\
\texttt{pathologic\_N}     & 4 & 548 & \best{0.4672} & 0.4416 & 0.4124 \\
\texttt{os\_binary}        & 2 & 548 & \best{0.7573} & 0.7372 & 0.6350 \\
\texttt{tumor\_status}     & 2 & 548 & 0.6734 & \best{0.6825} & 0.6296 \\
\bottomrule
\end{tabular}
\end{table}
...
single-seed (seed 42) test accuracies; the \texttt{cancer\_type} heads come from
\texttt{results/\{independent,main,mor\_token\}.json} and the five staging/survival tasks from
\texttt{results\_genonet\{\_independent,,\_mor\}/}. All three model types were trained on every
task; the page-1 table reports the same three-way comparison with full parameter and
multi-metric detail on \texttt{cancer\_type}.
