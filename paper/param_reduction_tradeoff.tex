...
\begin{table}[h]
\centering
\caption{\textbf{Benchmark composition.} The six TCGA tasks, their type, number of classes,
class distribution, and held-out test size.}
\label{tab:tasks}
\begin{tabular}{l l c l c}
\toprule
\textbf{Task} & \textbf{Target type} & \textbf{Classes} & \textbf{Class distribution} & \textbf{Test $n$} \\
\midrule
\texttt{cancer\_type}      & Tumor of origin       & 4 & 867 / 423 / 943 / 505  & 523 \\
\texttt{pathologic\_stage} & Stage I--IV           & 4 & 966 / 886 / 528 / 358  & 548 \\
\texttt{pathologic\_T}     & Tumor size T1--T4     & 4 & 700 / 1323 / 458 / 257 & 548 \\
\texttt{pathologic\_N}     & Nodal status N0--N3   & 4 & 1474 / 827 / 380 / 57  & 548 \\
\texttt{os\_binary}        & Overall survival      & 2 & 407 / 2331             & 548 \\
\texttt{tumor\_status}     & Tumor-free / w-tumor  & 2 & 2139 / 599             & 548 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{\textbf{SMART results across all six tasks} (shared recursive model, depth 4, seed 42).
``Majority'' is the always-predict-largest-class accuracy floor. Tumor-of-origin is
near-perfect. Clinical-staging and survival targets are intrinsically hard from bulk
expression: on the imbalanced binary tasks (\texttt{tumor\_status}, \texttt{os\_binary}) the
model trades raw accuracy below the majority floor for a much higher \emph{balanced} macro-F1,
while on the 4-class staging tasks it clears the floor on \texttt{pathologic\_stage}.}
\label{tab:results}
\begin{tabular}{l c r r r}
\toprule
\textbf{Task} & \textbf{Classes} & \textbf{Majority acc.} & \textbf{Test acc.} & \textbf{Macro-F1} \\
\midrule
\rowcolor{morblue} \texttt{cancer\_type} & 4 & 0.344 & \best{0.9847} & \best{0.9871} \\
\texttt{tumor\_status}     & 2 & 0.781 & 0.6825 & 0.5827 \\
\texttt{os\_binary}        & 2 & 0.851 & 0.7372 & 0.5696 \\
\texttt{pathologic\_stage} & 4 & 0.353 & 0.5000 & 0.4504 \\
\texttt{pathologic\_T}     & 4 & 0.483 & 0.4580 & 0.4150 \\
\texttt{pathologic\_N}     & 4 & 0.538 & 0.4416 & 0.3120 \\
\bottomrule
\end{tabular}
\end{table}
...
\vspace{0.3em}
\noindent\footnotesize
\textit{Notes.} All numbers are single-seed (seed 42) test-set results from
\texttt{results\_genonet/} (staging/outcome tasks) and the \texttt{cancer\_type} head in
\texttt{results/main.json}. Majority floors are computed from the class distributions in
Table~\ref{tab:tasks}. The parameter-reduction trade-off on page~1 is reported on
