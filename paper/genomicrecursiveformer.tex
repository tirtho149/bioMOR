...
\begin{tabular}{llccc}
\toprule
& & \multicolumn{3}{c}{Macro-F1 \% (mean\,$\pm$\,std, 5-fold CV)} \\
\cmidrule(lr){3-5}
Cohort & Task & Independent & Shared & MoR \\
...
\bottomrule
\end{tabular}
\caption{\textbf{Cohort-wise clinical prediction} (macro-F1 \%, mean\,$\pm$\,std over
stratified 5-fold CV). \texttt{cancer\_type} is the unified four-cohort model; the five
clinical / outcome targets are modeled separately within each cohort, for all three
model types. Recurrent-block parameter counts under each header. Best per row in
\textbf{bold}. Only tumor-of-origin and thyroid nodal status carry strong signal;
staging targets sit near chance, and the three model types are statistically
indistinguishable --- the parameter saving is preserved in the low-data regime.}
\label{tab:cohortcv}
\end{table}
