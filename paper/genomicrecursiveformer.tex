...
% --- tighter float / whitespace control (reduce vertical gaps) ---
...
\label{sec:biorouter}
We optionally move a biological prior \emph{into} the routing decision: the expert-choice
logit gains an additive, annealed bias,
\begin{equation}
\tilde r^{(t)}_m \;=\; \tfrac{1}{\tau}\,\mathbf{w}_r^{\top}\mathbf{H}^{(t)}_m \;+\; \beta_t\,\pi_m,
\label{eq:biorouter}
\end{equation}
where $\pi_m$ is the label-free co-expression-network centrality of gene $m$ (eigenvector
centrality of genomap's gene-gene interaction graph \cite{islam2023cartography}) and
$\beta_t$ decays to $0$, nudging co-expression hub genes to recurse deeper early in training
and letting the data-driven term take over late. Because $\pi$ is a
constant additive bias the gate stays differentiable and nothing about the
parameter-efficiency claim changes. We validate it against a degree-matched random-graph
control (only a win over that control implicates biology, not mere smoothing); full equations
and the control are in Appendix~\ref{sec:interaction}.
...
\paragraph{SMART matches foundation models at a fraction of the size.}
The headline claim is parameter efficiency: a 0.7M-parameter model should not have to
concede accuracy to 50--400M-parameter foundation models. We test this on the five harder
clinical / outcome heads against ten classical ML learners (Table~\ref{tab:baselines}) and
seven deep / foundation-model baselines (Table~\ref{tab:foundation}). The story is
consistent: SMART has the best average macro-F1 of every \emph{deep} model
(46.5\%, vs.\ Geneformer 45.9\% and Cell2Sentence 43.7\%) while using $72$--$569\times$ fewer
parameters, and is competitive with a heavily-tuned gradient-boosting ensemble --- at two to
three orders of magnitude fewer parameters and with an interpretable marker panel none of them
provide.
...
\subsection{Cohort-Wise Clinical Prediction}
Beyond tumor-of-origin, we ask whether the same architecture recovers clinical
staging and outcome \emph{within} each cohort. Tumor-of-origin (\texttt{cancer\_type})
is modeled unified across all four cohorts; the five clinical / outcome targets are
modeled cohort-by-cohort (breast, lung, thyroid, head\&neck), so staging and survival
are learned without cross-cohort confounding. Because the cohorts are small
(423--943 samples) and class-imbalanced, every target is evaluated with stratified
5-fold cross-validation and reported as macro-F1 (mean\,$\pm$\,std). Accuracy is
\emph{not} reported here: on these splits it collapses to the majority-class floor and
is bit-identical across the three model types, masking the true difficulty; macro-F1
exposes it. Table~\ref{tab:cohortcv} reports all three model types (Independent,
shared recursion, MoR) per cell.

Two findings stand out. First, only tumor-of-origin ($\sim$97\% macro-F1) and nodal
status in thyroid (\texttt{pathologic\_N}, 59--65\%) carry strong signal; the 4-class
staging targets hover near the 25\% chance level and the binary outcome targets near
50\%, confirming that fine-grained clinical staging is largely \emph{not} recoverable
from bulk expression at these cohort sizes. Second, no single model type dominates and
the three are mostly within one standard deviation of one another --- the
$\sim$4$\times$ parameter saving of shared recursion (and the additional compute saving
of MoR) is preserved with no systematic loss in this much harder, low-data regime.
...
\begin{tabular}{llccc}
\toprule
& & \multicolumn{3}{c}{Mean\,$\pm$\,std (metric per block)} \\
\cmidrule(lr){3-5}
Cohort / dataset & Task & Independent & Shared & MoR \\
& & {\scriptsize 530{,}432 p.} & {\scriptsize 132{,}992 p.} & {\scriptsize 132{,}992 p.} \\
\midrule
\multicolumn{5}{l}{\emph{TCGA bulk RNA-seq --- macro-F1 \%, 5-fold CV}}\\
\midrule
Unified & \texttt{cancer\_type} & \textbf{97.9\,$\pm$\,0.9} & 96.9\,$\pm$\,0.4 & 96.7\,$\pm$\,0.9 \\
\midrule
\multirow{5}{*}{Breast}
 & \texttt{path\_stage} & 26.4\,$\pm$\,3.8 & 25.1\,$\pm$\,3.9 & \textbf{26.6\,$\pm$\,3.2} \\
 & \texttt{path\_T}     & \textbf{26.6\,$\pm$\,4.4} & \textbf{26.6\,$\pm$\,3.7} & 24.8\,$\pm$\,2.3 \\
 & \texttt{path\_N}     & \textbf{22.8\,$\pm$\,1.3} & 21.2\,$\pm$\,2.6 & 20.7\,$\pm$\,4.6 \\
 & \texttt{os\_binary}  & 50.2\,$\pm$\,5.7 & \textbf{50.4\,$\pm$\,3.9} & 49.3\,$\pm$\,5.9 \\
 & \texttt{tumor\_stat} & \textbf{51.6\,$\pm$\,3.4} & 47.4\,$\pm$\,5.0 & 50.8\,$\pm$\,3.0 \\
\midrule
\multirow{5}{*}{Lung}
 & \texttt{path\_stage} & 26.1\,$\pm$\,2.5 & 24.2\,$\pm$\,2.9 & \textbf{27.8\,$\pm$\,1.6} \\
 & \texttt{path\_T}     & \textbf{27.3\,$\pm$\,2.2} & 24.7\,$\pm$\,1.6 & 22.4\,$\pm$\,3.3 \\
 & \texttt{path\_N}     & 22.7\,$\pm$\,2.0 & 24.1\,$\pm$\,3.5 & \textbf{24.5\,$\pm$\,2.5} \\
 & \texttt{os\_binary}  & 46.1\,$\pm$\,1.6 & \textbf{46.9\,$\pm$\,0.9} & 46.8\,$\pm$\,0.9 \\
 & \texttt{tumor\_stat} & \textbf{55.0\,$\pm$\,2.8} & 52.6\,$\pm$\,5.2 & 53.1\,$\pm$\,3.2 \\
\midrule
\multirow{5}{*}{Thyroid}
 & \texttt{path\_stage} & \textbf{25.3\,$\pm$\,3.4} & 24.5\,$\pm$\,4.0 & 24.6\,$\pm$\,4.7 \\
 & \texttt{path\_T}     & 24.8\,$\pm$\,5.9 & 24.5\,$\pm$\,3.3 & \textbf{25.3\,$\pm$\,2.4} \\
 & \texttt{path\_N}     & 58.0\,$\pm$\,3.4 & \textbf{64.8\,$\pm$\,2.7} & 58.9\,$\pm$\,4.9 \\
 & \texttt{os\_binary}  & \textbf{49.2\,$\pm$\,2.2} & 48.7\,$\pm$\,0.2 & 47.2\,$\pm$\,3.0 \\
 & \texttt{tumor\_stat} & \textbf{50.6\,$\pm$\,7.3} & 47.8\,$\pm$\,2.1 & 47.2\,$\pm$\,3.2 \\
\midrule
\multirow{5}{*}{Head\&neck}
 & \texttt{path\_stage} & 20.4\,$\pm$\,4.9 & \textbf{21.8\,$\pm$\,3.9} & 19.2\,$\pm$\,2.3 \\
 & \texttt{path\_T}     & \textbf{20.8\,$\pm$\,3.8} & 20.4\,$\pm$\,3.6 & 19.5\,$\pm$\,2.3 \\
 & \texttt{path\_N}     & 25.8\,$\pm$\,3.8 & 24.9\,$\pm$\,5.5 & \textbf{28.8\,$\pm$\,3.9} \\
 & \texttt{os\_binary}  & 47.5\,$\pm$\,0.2 & 47.9\,$\pm$\,1.6 & \textbf{48.7\,$\pm$\,2.3} \\
 & \texttt{tumor\_stat} & 50.8\,$\pm$\,4.7 & \textbf{51.2\,$\pm$\,4.0} & 50.0\,$\pm$\,2.1 \\
\midrule
\multicolumn{5}{l}{\emph{Single-cell genomap --- accuracy \%, 5 seeds (genoNet ref: TM 93, Pancreas 97)}}\\
\midrule
\multirow{2}{*}{Genomap}
 & \texttt{tabula\_muris} (55) & 82.5\,$\pm$\,2.7 & 82.1\,$\pm$\,2.2 & \textbf{84.3\,$\pm$\,1.1} \\
 & \texttt{pancreas} (15)      & 90.1\,$\pm$\,3.0 & \textbf{92.4\,$\pm$\,1.3} & 90.9\,$\pm$\,1.6 \\
\bottomrule
\end{tabular}
\caption{\textbf{Unified results across domains} (three model variants). \textbf{TCGA bulk
RNA-seq} (top): \texttt{cancer\_type} unified, the five clinical / outcome targets modeled
separately within each cohort; macro-F1 \%, mean\,$\pm$\,std over stratified 5-fold CV
(accuracy collapses to the majority floor on these small imbalanced cohorts).
\textbf{Single-cell genomap} (bottom): Tabula Muris recognition and pancreas integration under
the genomap protocol; accuracy \%, mean\,$\pm$\,std over 5 seeds, with genoNet's reported
accuracy for reference (TM 93\%, Suppl.\ S4; Segerstolpe pancreas 97\%, Fig.~7c). Recurrent-block
parameters under each header; best per row in \textbf{bold}; metric differs by block as labelled.}
\label{tab:cohortcv}
\end{table}

\subsection{Single-Cell Recognition vs.\ Genomap}
To position the recursive model against an external single-cell baseline, we run it
on the genomap datasets (Islam \& Xing, \emph{Nat.\ Commun.} 2023) under that paper's
exact protocol: precomputed genomap features (5--10\% HVG + entropy cartography), the
paper's train/test split (Tabula Muris 70/30; pancreas integration split), Adam with
weight decay (lr 0.001, weight decay $10^{-5}$, batch 128, 150 epochs), and accuracy as
the headline metric. We restrict to the two datasets genomap uses for \emph{classification}
--- Tabula Muris cell recognition and the five-dataset pancreas integration; the other
capsule matrices (sea-squirt developmental stages, retinal bipolar) were used in that
paper for trajectory and clustering/visualization, not classification, and are excluded.
These two datasets form the \emph{single-cell genomap} block at the bottom of
Table~\ref{tab:cohortcv} (reported as accuracy \%, mean\,$\pm$\,std over 5 seeds, alongside the
TCGA cohorts). genomap's purpose-built genoNet CNN --- which operates on the 2D genomap
\emph{image} --- is the stronger classifier (93\% on Tabula Muris, Suppl.\ Fig.~S4; 97\% on the
Segerstolpe pancreas subset), as expected of a model specialised to that input. Our general
recursive transformer, consuming the same \emph{flattened} genomap features, trails it (best
variant 84.3\% on Tabula Muris, 92.4\% on pancreas) but at \emph{two to three orders of
magnitude fewer parameters} --- the point is that the same 0.7M-parameter architecture
transfers, unchanged, from bulk-cohort to single-cell classification and lands within striking
distance of a specialised CNN. Over five seeds the three variants are within a point or two of
each other (MoR best on Tabula Muris, shared best on pancreas), preserving the parameter-saving
story of Section~\ref{sec:foundation}.
...
\paragraph{Token reduction.}\label{sec:tokenred} Both savings follow from one upstream effect: the router
keeps 256 of 4{,}000 gene tokens (93.6\% reduction, lowering attention cost
244.1$\times$, $O(N^2)\!\to\!O(M^2)$), and the retained tokens are the informative ones
(mean importance 34.13 vs.\ 11.96 discarded). Full statistics, the importance ranking and
a consistency analysis are in Appendix~\ref{app:tokenred}.
...
The headline is parameter footprint. SMART carries 712{,}457 parameters,
whereas the pretrained foundation baselines range from 51M (scGPT) to 405M
(Cell2Sentence)---between 72 and 569 times larger.
...
\midrule
\textbf{SMART (ours)} & \textbf{this work} & \textbf{712K} & \textbf{1$\times$} & \textbf{46.5} \\
\midrule
scGPT (pan-cancer)~\cite{cui2024scgpt} & Nat. Methods 2024 & 51.3M & 72$\times$ & 33.0 \\
CellPLM~\cite{wen2024cellplm} & ICLR 2024 & 82.4M & 116$\times$ & 39.0 \\
Geneformer (V2-104M)~\cite{theodoris2023transfer} & Nature 2023 & 104.4M & 147$\times$ & 45.9 \\
Cell2Sentence (Pythia-410M)~\cite{levine2024cell2sentence} & ICML 2024 & 405.3M & 569$\times$ & 43.7 \\
scGeneFit~\cite{dumitrascu2021optimal} & Nat. Commun. 2021 & n/a & n/a & 44.0 \\
sciLaMA~\cite{hu2025scilama} & ICML 2025 & -- & -- & 44.6 \\
scbenchmark (6L/256d)~\cite{qi2025scbenchmark} & AAAI 2025 & -- & -- & 27.3 \\
\bottomrule
\end{tabular}}
...
co-designed: the same mechanisms that make the model small (weight sharing, marker
compression) also make it interpretable. The headline finding is informatively mixed
--- SMART attains the best across-task mean macro-F1 of any deep model and wins the
binary clinical heads, but tuned gradient boosting leads on tumour stage and T --- so
SMART's advantage is \emph{efficiency and interpretability at competitive accuracy},
not a universal accuracy gain.

We scope the claims accordingly. (i) \emph{Marker learning is signal-dependent}: the
learned router clearly beats random/variance selection on the cohort task, but on the
weakly-determined staging labels every component sits within one std of the others.
(ii) \emph{Accuracy ceiling}: closing the staging gap to gradient boosting likely needs
ordinal-aware losses or pretraining \cite{cui2024scgpt,hao2024large,fu2025get}.
(iii) \emph{Generalisation}: results are single-assembly (TCGA); leave-one-cohort-out,
an external cohort, and efficient-attention backbones
\cite{wang2020linformer,choromanski2021rethinking,xiong2021nystromformer} remain future
work. (iv) \emph{Depth vs.\ variance}: recursion depth and a gene's expression variance
may be partially entangled, so part of the depth signal could reflect statistical
prominence rather than biology; we do not disentangle them here. (v) \emph{Biology-informed
routing is not yet realised}: the genomap interaction prior does not separate from a
degree-matched random-graph control on our tasks (App.), so we claim no benefit from
biological structure. Efficiency evidence is stack FLOPs and wall-clock rather than
end-to-end inference profiles; we report these so the trade-offs are not overstated.
...
\section{Clinical Phenotype Tasks (genoNet Benchmark)}
...
\section{Router-Based Gene Identification}
\label{sec:geneid}
...
Having established the headline result --- parameter-efficient accuracy competitive with
much larger models --- we now isolate \emph{which} design choices are responsible. Each
study below changes one component and holds the rest fixed: the recursion routing
(Table~\ref{tab:routing}), early-exit vs.\ fixed depth (Table~\ref{tab:earlystop}), the
marker selector (Table~\ref{tab:selection}), router initialisation and annealing
(Table~\ref{tab:initanneal}), biology-informed routing (Table~\ref{tab:interaction}), the
core architecture (Table~\ref{tab:ablation}), and robustness over seeds on the harder heads
...
\midrule
Fixed depth ($K{=}8$) & 97.4\,$\pm$\,0.4 & 45.9\,$\pm$\,0.5 & 38.7\,$\pm$\,0.6 & 28.8\,$\pm$\,9.8 & 8.00 & 1.00$\times$ \\
Early-exit ($K{=}8$, ours) & 97.7\,$\pm$\,0.8 & 47.3\,$\pm$\,2.5 & 36.0\,$\pm$\,5.1 & 28.1\,$\pm$\,3.8 & 4.75 & 0.49$\times$ \\
\bottomrule
\end{tabular}}
...
The router reduces the 4,000 gene tokens of a sample to 256 marker
tokens, a 93.6\% reduction (Table~\ref{tab:tokenred}). Because
self-attention is quadratic in the token count, this lowers the attention cost by a
factor $(N/M)^2 = $244.1$\times$, which is the upstream source of the
parameter and FLOP savings reported in the main paper.

\paragraph{(2) Token selection information.}
The selected (retained) tokens are the 256 genes chosen by the $M$ marker
...
score, max over the $M$ slots), giving a continuous ranking over all 4,000 genes.
...
disproportionately important: their mean importance is 34.13 against
11.96 for the discarded tokens, and they hold 16.3 of the total
importance mass while being a 93.6\%-smaller set, so selection is far
from random. Second, a gene's selection importance and the compute it is subsequently
allocated are nearly uncorrelated in rank (Spearman $\rho{=}$-0.062 between
...
$\rho$(importance, recursion depth) & -0.062 \\
...
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{rlc@{\hskip 1.5em}rlc}
\toprule
\multicolumn{3}{c}{Most important (descending)} & \multicolumn{3}{c}{Least important (ascending)} \\
