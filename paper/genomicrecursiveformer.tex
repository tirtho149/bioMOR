...
qualitatively on the headline cohort model. The router keeps 256 marker
tokens out of 4,000 gene tokens, a 93.6\% reduction that lowers
the self-attention cost by 244.1$\times$ ($O(N^2)\!\to\!O(M^2)$;
...
carry 16.3 of the total importance mass and have a mean importance of
34.13 versus 11.96 for the discarded tokens, so the router keeps
...
in rank (Spearman $\rho{=}$-0.062 between marker affinity and mean recursion depth):
...
shrinks the 4,000-gene input, whether the kept tokens are the informative ones
